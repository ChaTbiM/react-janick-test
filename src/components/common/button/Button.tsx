...
const Button: FC<ButtonProps> = ({ theme, text, ...restOfProps }) => {
  const defaultClasses =
    'flex justify-center items-center mt-4 px-1.5 py-1 text-white text-center capitalize'
  const classesForPrimary = 'bg-blue-500'
  const classesForSecondary = 'bg-red-500'
  const isButtonDisabled = restOfProps.disabled ?? false
  return (
    <button
      type="button"
      {...restOfProps}
      ...
      } ${restOfProps.className} ${isButtonDisabled ? 'bg-blue-300' : null}`}
    >
      {isButtonDisabled && (
        <svg
          aria-hidden="true"
